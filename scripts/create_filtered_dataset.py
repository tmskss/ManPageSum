...
DATASET_FILENAME = 'dataset_filtered.csv'
...
for command in command_names:
    if command not in tldr_dict:
        continue
    data.append((command, man_pages_dict[command], tldr_dict[command]))


with open(DATA_DIR + DATASET_FILENAME, 'w', newline='') as csv_file:
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['Command', 'Text', 'Summary'])  # Writing header row
